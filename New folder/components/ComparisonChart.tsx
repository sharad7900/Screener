...
const ComparisonChart: React.FC<ComparisonChartProps> = ({ funds }) => {
  ...
  // Logic to align data for multi-line chart
  // In a real app, you would fetch normalized time-series data from an API
  // Here we assume the mock history arrays are roughly aligned by date
  const chartData = useMemo(() => {
    // Determine slice length based on range (Mock logic)
    let days = 365;
    if (range === '5D') days = 5;
    if (range === '1M') days = 30;
    if (range === '6M') days = 180;
    if (range === '1Y') days = 365;
    // For simplicity, using first fund's dates as x-axis
    ...
    const slicedData = primaryFund.history.slice(-days).map((point, index) => {
      ...
      funds.forEach(f => {
        // Safe access to history index
        // In real app: align by exact date key
